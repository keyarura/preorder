...
class ReceivablePayableReport(object):
	...
	def get_columns(self, party_naming_by, args):
		...
		# columns += [_("Voucher Type") + "::110", _("Voucher No") + ":Dynamic Link/"+_("Voucher Type")+":120", _("Tax No") + ":Data:150", _("Resi Number") + ":Data:150",
		...
		for gle in self.get_entries_till(self.filters.report_date, args.get("party_type")):
			if self.is_receivable_or_payable(gle, dr_or_cr, future_vouchers):
				...
				if abs(outstanding_amount) > 0.1/10**currency_precision:
					...
					# get resi number
					# resi_number = voucher_details.get(gle.voucher_no, {}).get("resi_number", "")

					# get date of receipt
					receipt_date = voucher_details.get(gle.voucher_no, {}).get("date_of_receipt", "")

					# row += [gle.voucher_type, gle.voucher_no, tax_no, resi_number, receipt_date, due_date]
	...
	def get_voucher_details(self, party_type):
		...
		if party_type == "Customer":
			# for si in frappe.db.sql("""select name, due_date, tax_no, date_of_receipt,resi_number
