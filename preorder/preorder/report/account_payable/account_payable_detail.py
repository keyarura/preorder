...
class ReceivablePayableReport(object):
	...
	def get_columns(self, party_naming_by, args):
		...
		# columns += [_("Voucher Type") + "::110", _("Voucher No") + ":Dynamic Link/"+_("Voucher Type")+":120", _("Tax No") + ":Data:150", _("Switch Charge Method") + ":Data:150",
		#       _("Due Date") + ":Date:80"]
	...
	def get_voucher_details(self, party_type):
		...
		if party_type == "Customer":
			for si in frappe.db.sql("""select name, due_date, tax_no, date_of_receipt
				from `tabSales Invoice` where docstatus=1""", as_dict=1):
					voucher_details.setdefault(si.name, si)

		if party_type == "Supplier":
			# for pi in frappe.db.sql("""select name, due_date, bill_no, bill_date,tax_no,switch_charge_method
