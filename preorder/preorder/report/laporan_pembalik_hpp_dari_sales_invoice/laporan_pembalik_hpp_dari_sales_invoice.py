...
def execute(filters=None):
	...
	sl_entries = frappe.db.sql("""select si.`name`, si.posting_date, si.net_total, sii.sales_order from `tabSales Invoice` si inner join `tabSales Invoice Item` sii on si.`name` = sii.parent where si.docstatus = '1' and sii.sales_order is not null %s order by si.posting_date asc""" % conditions, as_dict=1)
